...
# El área sombreada representa el intervalo de confianza del 95 por ciento alrededor de la línea
# de regresión.
...
# Después de un análisis de regresión, es una buena idea graficar los residuales frente a los
# valores predichos. Si los residuales forman un patrón aleatorio alrededor de una línea
# horizontal en cero, eso es evidencia a favor de una relación lineal entre la variable
# independiente y la variable dependiente.
# La figura 2 muestra el gráfico de residuos del ejemplo. El patrón de residuos alrededor de la
# línea es consistente con un modelo lineal.

# El plot se basa en FM.reg, el modelo lineal. Aquí está la declaración de ggplot():
ggplot(FM.reg, aes(x=fitted(FM.reg), y=residuals(FM.reg)))
# Las asignaciones x e y se basan en la información del análisis. Como puede adivinar,
# fitted(FM.reg) recupera los valores predichos y residuals(FM.reg) recupera los residuales.

# Para trazar puntos, agrega la función geom apropiada:
geom_point()

# y luego una función geom para la línea horizontal discontinua cuya intersección y es 0:
geom_hline(yintercept = 0, linetype = "dashed" )


# Entonces el código de la figura 2 es
ggplot(FM.reg, aes(x=fitted(FM.reg), y=residuals(FM.reg)))+ geom_point() + geom_hline(yintercept = 0, linetype = "dashed" )
