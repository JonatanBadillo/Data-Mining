...
NINFAS2023_mes_contaminantes <- ggplot(promedio_mes, aes(x = Mes, y = Promedio_Concentracion, color = Contaminante, group = Contaminante)) +
  geom_line(size = 1) +
  facet_wrap(~ Contaminante, scales = "free_y") +
  labs(title = "Promedio de concentración de contaminantes por mes en Puebla UTP 2023",
       x = "Mes del año",
       y = "Promedio de concentración") +
  theme_minimal()

# Guardar el gráfico en la carpeta 'imagenes'
ggsave("~/Desktop/UNIVERSITY/Servicio-Social/Data-Mining/SeriesTiempo-contaminantes/imagenes/mes/UTP2023/Todos-Contaminantes/UTP2023_mes_contaminantes.jpg", NINFAS2023_mes_contaminantes, width = 10, height = 6, dpi = 300)
...
# Ruta base para guardar los gráficos
ruta_base <- "~/Desktop/UNIVERSITY/Servicio-Social/Data-Mining/SeriesTiempo-contaminantes/imagenes/mes/UTP2023/Cada-Contaminantes"
...
for(contaminante in contaminantes_unicos) {
  ...
  if (nrow(datos_contaminante) > 1) {
    ...
    grafico <- ggplot(datos_contaminante, aes(x = Mes, y = Promedio_Concentracion, group = 1)) +
      geom_line(size = 1) +
      scale_x_discrete(limits = month.abb) + # Asegura que se muestren todos los meses
      labs(title = paste("Promedio de concentración de", contaminante, "por mes en Puebla UTP 2023"),
           x = "Mes del año",
           y = "Promedio de concentración") +
      theme_minimal()
    
    # Definir el nombre del archivo basado en el contaminante
    nombre_archivo <- paste(ruta_base, "/UTP2023_mes_", tolower(gsub(" ", "_", contaminante)), ".jpg", sep = "")
